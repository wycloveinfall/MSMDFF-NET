# -*-coding:utf-8-*-
# !/usr/bin/env python
# @Time    : 2023/3/22 下午3:45
# @Author  : 王玉川 uestc
# @File    : Massachusetts_loader.py
# @Description :
##+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
def create_train_list(root_dir,train_sample_path,train_size,random_state):
    # root_dir = '/home/wyc/mount/DEEPLEARNING_DATA/数据仓库/公开的道路数据/Massachusetts Roads/tiff'
    # train_sample_path = '/home/wyc/mount/SSD_AREA/仿真数据/New_Net/dataset_split_text/Massachusetts/'
    # train_size = 0.85
    # random_state = 8
    ...
    if len(os.listdir(train_sample_path)) == 0:
        ...
        lab_dir = root_dir + '/train_labels'
        image_srcList = getDatapath(img_dir, dir_command='*', key_word='.tiff')
        image_labList = getDatapath(lab_dir, dir_command='*', key_word='.tif')
        image_srcList.sort()
        image_labList.sort()

        image_data_list = ['%s @%s' % (mess1, mess2) for mess1, mess2 in zip(image_srcList, image_labList)]
        data_check_for_Massachusetts(image_data_list)  # 这行代码具体的校验方式，还应该根据具体的实际情况而定
        trainlist, \
...
def Massachusetts_data_512_512(src_img, lab_img, imageName, test_flage=False):
    size_h = 512
    size_w = 512
    # 随机剪切
    center_x = np.array([256, 512, 768,1024,1280])
    center_y = np.array([256, 512, 768,1024,1280])
    angle_ = cfg_data_api.TRAIN_SETTING['ROTATE']
    if  test_flage == False:
        src_img_pad = np.zeros([3,1536,1536],dtype=np.uint8)
        lab_img_pad = np.zeros([1536,1536],dtype=np.uint8)
        src_img_pad[:,18:-18,18:-18] = src_img
        lab_img_pad[18:-18,18:-18] = lab_img
        ...
        rand_y = 750
        rand_angle = 0
    img = src_img_pad[:,rand_x-size_h//2:rand_x+size_h//2,rand_y-size_w//2:rand_y+size_w//2]
    label = lab_img_pad[rand_x-size_h//2:rand_x+size_h//2,rand_y-size_w//2:rand_y+size_w//2]
...
########################################################################################################################
class Massachusetts_dataloader():
    def __init__(self):
        if cfg_data_api.DATA_SETTING['deep_lodader_type'] == '512_512':
            self.deepGlobe_data_load_Type = Massachusetts_data_512_512
        else:
    ...
    def _loader_deepGlobe(self, data_list, index, data_dict=[None], class_num=1, input_normalization=False,
        ...
        try:
            if mask.sum() < 1024 * 255: #去除大面积没有道路的标签
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, train_list, readType='v1', class_num=1,
        ...
        self.loader = Massachusetts_dataloader()._loader_deepGlobe
