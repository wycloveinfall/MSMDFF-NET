...
class DecoderBlock(nn.Module):
    ...
    def h_transform(self, x):
        shape = x.size() # 2 64 64 64
        x = torch.nn.functional.pad(x, (0, shape[-1])) # 2 64 64 128
    ...
    def inv_h_transform(self, x):
        shape = x.size() # 2*32*64*127
        x = x.reshape(shape[0], shape[1], -1).contiguous() # 2*32*8128
        x = torch.nn.functional.pad(x, (0, shape[-2])) # 2*32*8192(+64)
...
class DecoderBlock_USE_SCMD(nn.Module):
    def __init__(self, in_channels, n_filters, BatchNorm, inp=False):
        super(DecoderBlock_USE_SCMD, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, in_channels // 4, 1)
        self.bn1 = BatchNorm(in_channels // 4)
        self.relu1 = nn.ReLU()
        self.inp = inp
        ...
        #     in_channels // 4, in_channels // 8, (1, 9), padding=(0, 4)
        # )
        self.deconv3 = nn.Conv2d(in_channels // 4, in_channels // 8, (1, 9),padding=(0, 4))
    ...
    def forward(self, x, inp = False):
        ...
        # x3 = self.inv_h_transform(self.deconv3(self.h_transform(x)))
        # x4 = self.inv_v_transform(self.deconv4(self.v_transform(x)))
        x3 = tensor_rotate(self.deconv3(tensor_rotate(x,self.angle[1])),-self.angle[1])
        x4 = tensor_rotate(self.deconv4(tensor_rotate(x,self.angle[3])),-self.angle[3])
        x = torch.cat((x1, x2, x3, x4), 1)
        if self.inp:
            x = F.interpolate(x, scale_factor=2)
        x = self.bn2(x)
...
class Decoder_USE_SCMD(nn.Module):
    def __init__(self, num_classes, backbone, BatchNorm):
        super(Decoder_USE_SCMD, self).__init__()
        if backbone == 'resnet':
            in_inplanes = 256
        else:
            raise NotImplementedError

        self.decoder4 = DecoderBlock_USE_SCMD(in_inplanes, 256, BatchNorm)
        self.decoder3 = DecoderBlock_USE_SCMD(512, 128, BatchNorm)
        self.decoder2 = DecoderBlock_USE_SCMD(256, 64, BatchNorm, inp=True)
        self.decoder1 = DecoderBlock_USE_SCMD(128, 64, BatchNorm, inp=True)
...
def build_decoder_use_SCMD(num_classes, backbone, BatchNorm):
    return Decoder_USE_SCMD(num_classes, backbone, BatchNorm)